...
$users = $conn->query("SELECT * FROM users WHERE role IN ('asisten_dosen', 'admin') OR id NOT IN (SELECT user_id FROM asisten_dosen WHERE user_id IS NOT NULL) ORDER BY nama");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($nama)) {
        $stmt = $conn->prepare("INSERT INTO asisten_dosen (koordinator_id, nama, email, no_hp) VALUES ( ?, ?, ?, ?)");
        $stmt->bind_param("isss", $koordinator_id, $nama, $email, $no_hp);
        ...
        if ($stmt->execute()) {
            ...
            $message = 'Gagal menambahkan asisten!';
            $messageType = 'error';
        }
        $stmt->close();
    } else {
...
<head>
    ...
    <title>Tambah Asisten - Admin</title>
